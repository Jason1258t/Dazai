import discord, time, random, requests, json, sqlite3
...
settings = {
    ...
    'bot': 'Dazai',
    'id': ,
    'prefix': ['дазай, ', 'Дазай, ', 'дазай ', 'Дазай ', 'Албанец, ', 'албанец, ', 'албанец ', 'Албанец ', 'даз ', 'daz ', 'Даз ', 'Daz ', 'dazai', 'даза', 'Даза'],
...
async def ало(ctx):
	...
	embed.add_field(name='если команды нет', value = 'выведет сообщение о том что этой команды нет')
...
async def картинки(ctx):
	emb = discord.Embed(title='Работа с предупреждениями', color=0xffdd00)
	emb.add_field(name='дазай, cat/dog/fox', value='отправляет рандомного котика/песеля/лису', inline=False)
...
@bot.command()
async def нюхай(ctx, text):
	if text == 'бебру':
		await ctx.send('сам нюхай')
